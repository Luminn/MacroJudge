...
import scan
import xmlreader
from names import COMPILERS_INFO, COMPILERS_DISPLAY_NAMES, compiled_compilers, bytecode_compilers, standard_interpreters
...
def compile_onclick(btn, text):
    ...
    errcount = 0
    time = 1
    if COMPILER in compiled_compilers:
        time = CURRENT_PROBLEM.times[0]
    elif COMPILER in bytecode_compilers:
        time = CURRENT_PROBLEM.times[1]
    else:
        time = CURRENT_PROBLEM.times[2]
    try:
        results = compile.excecute_source(text.get_text(text.get_start_iter(), text.get_end_iter()), COMPILER, map(lambda x : x.stdin, CURRENT_PROBLEM.tests), timeout=time)
