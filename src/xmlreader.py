...
class Problem:
    ...
    def parse_times(self, elem):
        self.times = [0, 0, 0]
        self.times[0] = int(elem.find('ctime').text)
        self.times[1] = int(elem.find('btime').text)
        self.times[2] = int(elem.find('itime').text)
    ...
    def parse(elem):
        ...
        if not elem.find("time-easy") is None:
            result.times = [1, 1, 1]
        elif elem.find('time-normal') is not None:
            result.times = [5, 10,15]
        else:
